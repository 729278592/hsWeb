...
function _init_area(){  //初始化函数
	$("#s_province li").remove();
	for(q=0;q<dsy.Items["0"].length;q++){
		$("#s_province").append($("<li onclick='changeCity("+q+")'>"+dsy.Items["0"][q]+"</li>"));
	}
	$(".adress_name li").css({"color":"#505050","width":"33.3%"});
	$(".adress_name li").css("border-bottom","0.1rem #e5e5e5 solid");
	$("#city").css("color","#FF620D");
	$("#city").css("border-bottom","0.1rem #ff620d solid");
	$("#citylist").show();
}
function changeCity(citynum){
	$(".adress_list").hide();
	i=citynum;
	$("#city").html(dsy.Items["0"][i]);
	$("#s_city li").remove();
	for(q=0;q<dsy.Items["0_"+i].length;q++){
		$("#s_city").append($("<li onclick='changeTown("+q+")'>"+dsy.Items["0_"+i][q]+"</li>"));
	}
...
function changeTown(townnum){
	$(".adress_list").hide();
	j=townnum;
	$("#town").html(dsy.Items["0_"+i][j]);
	$("#s_county li").remove();
	for(q=0;q<dsy.Items["0_"+i+"_"+j].length;q++){
		$("#s_county").append($("<li onclick='changeArea("+q+")'>"+dsy.Items["0_"+i+"_"+j][q]+"</li>"));
	}
...
function changeArea(areanum){	
	a=areanum;
	$("#area").html(dsy.Items["0_"+i+"_"+j][a]);
	readInfo($("#city").text(),$("#town").text(),$("#area").text());
}
