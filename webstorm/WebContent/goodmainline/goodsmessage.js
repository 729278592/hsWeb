	...
	$.ajax({
		...
		success : function(result){
			...
			if(result.gameGood[6] == "线上" || result.gameGood[6] == "都有"){
				//判断是否有返现金
				if(result.gameGood[3]>0){
					bq +='<a href="javascript:void(0);">' +
				 			'<div class="play">购买返现'+result.gameGood[3]+'元</div>' +
				 		 '</a>';
				}
				//判断是否有返积分
				if(result.gameGood[10]>0){
					bq +='<a href="javascript:void(0);">' +
				 			'<div class="ring">购买返'+result.gameGood[10]+'积分</div>' +
				 		 '</a>';
				}
				//判断是否有推荐奖励
				if(result.gameGood[4]>0){
					bq +='<a href="javascript:void(0);">' +
				 			'<div class="ring" style="background: #389ffe;">推荐最高奖励'+result.gameGood[4]+'元</div>' +
				 		 '</a>';
				}
				if(result.gameGood[3]==0&&result.gameGood[10]==0&&result.gameGood[4]==0){
					//bq +='<span style="float: left; color: #505050;font-size: 14px;line-height: 21px;height: 24px;margin-left: 18px;">抱歉，暂时没有相关活动!</span>';
					$('.midll-div').hide();
				}
			}
			...
			for(var i = 0;i < result.postObjs.length;i ++){
				...
				//计算星级,几颗星星
				var _stars='';
				if(result.postObjs[i][14]==1){
					_stars='one';
				}else if(result.postObjs[i][14]==2){
					_stars='two';
				}else if(result.postObjs[i][14]==3){
					_stars='three';
				}else if(result.postObjs[i][14]==4){
					_stars='four';
				}else if(result.postObjs[i][14]==5){
					_stars='five';
				}
			...
			var hpl = "";
			    hpl += '<li class="appraise">评价</li>' + 
		               ...
		               '<li class="people"><a style="color: #9e9e9e; display: block;" href="../goodml/goToGoodsPost.do?goodsId='+goodsId+'">'+result.postCount[1]+'人评价</a></li>' +
		               '<li class="topdiv-rt">' +
...
//				alert("code="+res);
	...
	wx.ready(function(){
		...
		//old request var fxUrl = share_url + "/secondbk2.html?fId=" + uid + "&t=p_" + gi + "&showUrl=http://"+hs_url+"/mxj/goodml/goToShareGoodsImg.do?goodsId="+gi;
		var fxUrl = share_url + "/secondbkAll.html?fId=" + uid + "&t=p_" + gi + "&type=" + share_type_good + "&showUrl=http://"+hs_url+"/mxj/goodml/goToShareGoodsImg.do?goodsId="+gi;
