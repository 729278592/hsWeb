...
var dataSum=0;
var shopp=0;
var showFlag = false;//第一次通过周边标签进入不显示坐标文本框。
...
function bianhao(){
	if(dataSum==0 || arrays.length==0 || arrays.length<dataSum){
		...
		//begin显示当前用户最近门店有销售情况得门店,如都没有销售情况就显示最近得门店=============
		//进行解码(汉字解码)
		cityName = decodeURI(decodeURI(cityName));
		
		var shopnum = "";
		var showNearShopFlag = false;//显示最近门店状态
		var shopIdsList = null;
		
		//查找该商品是否有销售情况,并显示最近得门店卡片展示出来
		$.ajax({
			url : '../goodml/shopSaleNear.do',
		...
		if(shopIdsList != null){
			for(var i = 0;i < arrays.length;i ++){
				for(var j = 0;j < shopIdsList.length;j ++){
					//去循环找地图上面得门店是否有销售情况
					if(maps2[arrays[i]] == shopIdsList[j]){
						shopnum = shopIdsList[j];
						showFlag = true;//展示门店卡片
						showNearShopFlag = true;
						break;
					}
				}
				//防止
		...
		//如用户所有位置附近都没有销售商品得情况
		if(!showNearShopFlag){
			shopnum = maps2[arrays[0]+""];
		}
		
		//end显示当前用户最近门店有销售情况得门店,如都没有销售情况就显示最近得门店=============
...
function showtan(shopnum,th,act){
    ...
    s=(map.getDistance(point,point2)/1000).toFixed(2);
			...
			if(obj && obj.shop){
				...
				if(obj.seller){
					...
					if(obj.seller.m_seller_pic!=null){
						contents="<div class='alertdiv'><span class='leftdiv'><img class='leftimg' src='../seller/smallDown.do?filename="+obj.seller.m_seller_pic+"' onclick='tosellerpage("+obj.seller.m_seller_id+")'/><br><span>"+sellerName+"</span></span><span class='rightdiv'><span class='stroename'>"+obj.shop.m_shopName+"</span><br/><span id='showstar"+shopnum+"'>";	
					}else{
 	 		  ...
 	 		  $.post("../shop/shopgoods.do?shopNo="+shopnum,function(obj2){
 	 				if(obj2 && obj2.shopgoods){
 	 					$.each(obj2.shopgoods,function(i,o){
 	 						//$('.activity').css("padding-bottom","1em");
 	 						//$("#goodlist").append($("<div class='swiper-slide'><div class='good' ><div class='goodleft'><div class='gamename' id='gamename"+i+"' style='margin-top:-2%;'></div><div class='lefttime' style='margin-top:-2%;' id='left"+i+"'><div class='hour' id='h"+i+"'>00</div><div class='ff' style='background-color: white;color:black;'>:</div><div class='minute' id='m"+i+"'>31</div><div class='ff' style='background-color: white;color:black;'>:</div><div class='seconds' id='s"+i+"'>45</div></div><div class='goodmsg'><span class='xinghao'><strong>"+o.m_good_no_short+"</strong></span><br/><span class='goodname' style='font-size:1em;'>"+o.m_good_name_short+"</span><br/><span class='gooddes' style='font-size:0.8em;'>"+o.m_good_des+"</span><br/><span class='goodprice' style='font-size:1em;'>￥"+o.m_good_price+"</span><br/></div></div><div class='goodright'><img src='../good/down.do?filename="+o.m_good_pic+"' class='goodimg' style='margin-top:-2%;'/></div></div></div>"));
 	 ...
 	 // $("img[src='http://api0.map.bdimg.com/images/iw_close1d3.gif']").attr("src","");
}
	...
	if(shopNo==null){
		...
		$.post(url,function(obj){
			if(obj && obj.shoplist){
				...
				Message.showNotify("您所在城市没有HC门店",3000);
			}
		},'json');
	}
}
