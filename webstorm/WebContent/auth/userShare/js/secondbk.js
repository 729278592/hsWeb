...
$(function() {
    ...
    var qrImg_url = mpServerPath + '/qr/getShareQr?uid='+fromId;//alert('/qr/getShareQr?uid='+fromId);
    ...
    core.requireJS(['../js/jweixin-1.0.0.js', '../js/mpAPI.js'], function() {
        ...
        mp.oAuthAPI(function(data) {
            ...
            var payData = JSON.stringify({"fromUid":fromId,"relId":id,"type":"info","openId":openid});
            $.ajax({
						 type: "POST",
					     url: sh_url,
			...
			//点击头条增加积分，由李鹏飞负责处理
			var addZenUrl = partnerServerPath + "/getZen4Weixin/addZen.do?type=" + share_type_info + "&uid="+fromId+"&infoId="+id;
			if( !checkEmptyString(openid) ){
				
				//增加点击者的opid
				addZenUrl += "&openId=" + openid;  //alert("&clickOpenId=" + openid);
				
				//最新需求：微信客户端分享头条，不送积分
//				//检查点击者的openid是否和发帖人的一致
//				if(!checkEmptyString(fromId)){
//					$.ajax({
//						async: false,
//						url: partnerServerPath + "/getZen4Weixin/getUserOpenId.do?uid="+fromId,
//						type: "get",
//						dataType : "json",
//						success: function (data) {
//							//alert("data.openId=" + data.openId + "||openid=" + openid );
//							//alert(data.openId != openid);
...
//				}
            ...
            mp.jsAPI(function() {
                ...
                var shareURL = staticServicePath + '/secondbkAll.html?fId=' + fromId + '&t='+t +'&showUrl='+ encodeURIComponent(showUrl) + "&type="+share_type_info;
